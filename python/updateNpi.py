...
#             row_data.append(hcp)
            ...
            npi = values[rec_npi]
            reqst = requests.get('http://www.bloomapi.com/api/search?limit=100&offset=0&key1=npi&op1=eq&value1='
                            + npi)
            if reqst:
                ...
                for i in range(len(result_set)):
                    ...
                    if result_set[i]['type'] == 'individual':
                        ...
                        values[rec_lname] = result_set[i]['organization_official']['last_name']
                        values[rec_fname] = result_set[i]['organization_official']['first_name']
                        values[rec_add1] = result_set[i]['practice_address']['address_line']
                        values[rec_add2] = utils.getAddressDetails(result_set, i)
                        values[rec_city] = result_set[i]['practice_address']['city']
                        values[rec_state] = result_set[i]['practice_address']['state']
    ...
    def loadFile(self):
        self.inputFileName = tk.filedialog.askopenfile(parent=self.fileBar, mode='rb', initialdir="../../Actelion/OriginalData/2015", title='Please choose the input file')
    ...
    def getSaveToDirectory(self):
        self.saveToDirectory = tk.filedialog.askdirectory(parent=self.fileBar, initialdir="../../Actelion/MissingData/2015", title='Please select a directory')
